...
const PROJECT_TYPES = [
  ...
  },
  {
    id: 'nodejs',
    name: 'Node.js',
    description: 'Pure Node.js application (Express, Next.js, etc.)',
    icon: NodeJsIcon,
    iconColor: 'text-green-500',
    features: ['Express', 'Next.js', 'Fastify'],
  },
  {
    id: 'custom',
    name: 'Custom PHP',
...
function CreateProject() {
  ...
  const hasRequiredBinaries = () => {
    if (formData.type === 'nodejs') {
      // Node.js projects only need Node.js binary (and a web server to proxy)
      return binariesStatus.nodejs.length > 0 && (binariesStatus.nginx.length > 0 || binariesStatus.apache.length > 0);
    }
      ...
      <div className="space-y-6">
        ...
        {formData.type === 'nodejs' && (
          <div className="space-y-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl">
              ...
              {(binariesStatus.nodejs || []).length === 0 ? (
                <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                  <div className="flex items-center gap-2 text-amber-700 dark:text-amber-300">
                    ...
                    <span className="text-sm font-medium">No Node.js versions installed</span>
                  </div>
                  <p className="text-sm text-amber-600 dark:text-amber-400 mt-1">
                    Please install at least one Node.js version from the Binary Manager.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-6 gap-2">
                  {(binariesStatus.nodejs || []).map((version) => (
                    <button
                      key={version}
                      onClick={() => updateFormData({ services: { ...formData.services, nodejsVersion: version, nodejs: true } })}
                      className={clsx(
                        'py-3 px-4 rounded-lg border-2 font-medium transition-all text-sm',
                        formData.services.nodejsVersion === version
            ...
            {/* App Port */}
            <div>
              <label className="label text-green-700 dark:text-green-300">Application Port</label>
              <input
                id="nodejs-port"
                type="number"
                min="1024"
                ...
                onChange={(e) => updateFormData({ nodePort: parseInt(e.target.value, 10) || 3000 })}
                className="input w-40"
                placeholder="3000"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Internal port your Node.js server listens on. Nginx will proxy requests to this port.
              </p>
            </div>

            {/* Start Command */}
            <div>
              <label className="label text-green-700 dark:text-green-300">Start Command</label>
              <input
                id="nodejs-start-command"
                type="text"
                value={formData.nodeStartCommand}
                onChange={(e) => updateFormData({ nodeStartCommand: e.target.value })}
                className="input font-mono"
                placeholder="npm start"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Command used to start your application (e.g., <code>npm start</code>, <code>npm run dev</code>, <code>node server.js</code>).
              </p>
            </div>
          </div>
        )}
